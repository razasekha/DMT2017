...
noiset_values = noiset_values.reshape((3,200,-1))
...
def standardize_set(arr):
    a3 = np.multiply(np.add(arr[0,:],-250),0.01)
    a4 = np.multiply(np.add(arr[1,:],-250),0.01)
    a5 = np.multiply(np.add(arr[2,:],-250),0.01)
    return np.array([a3,a4,a5])

six, threehundred, total_size = input_values.shape
new_in = np.zeros([3,200,total_size])
new_n = np.zeros([3,200,87000])
new_nt = np.zeros([3,200,7000])
new_st = np.zeros([3,200,512])
new_tst = np.zeros([3,200,512])
for j in range(total_size):
    new_in[:,:,j] = standardize_set(input_values[:,:,j])
for j in range(87000):
    new_n[:,:,j] = standardize_set(noise_values[:,:,j])
for j in range(7000):
    new_nt[:,:,j] = standardize_set(noiset_values[:,:,j])
for j in range(512):
    new_st[:,:,j] = standardize_set(specif_train_values[:,:,j])
for j in range(512):
    new_tst[:,:,j] = standardize_set(test_values[:,:,j])
batch_size = 1000

train_x = np.concatenate((new_in,new_n),2)
train_d = np.concatenate((desired_values,noised_values),0)
tr_x = np.transpose(train_x,[2,1,0])
ptr_x = np.transpose(new_st,[2,1,0])
tst_x = np.transpose(new_tst,[2,1,0])
tstn_x = np.transpose(new_nt,[2,1,0])
noise_values = np.transpose(new_n,[2,1,0])
#now the input are of size [batch_size, n_input]

learning_rate = tf.placeholder("float")
training_iters = 2000000 #2000000
display_step = 5

# Network Parameters
n_input = 200 # input size
...
weights = {
    ...
    'wd2': tf.Variable(tf.random_normal([48*64, 1024])),
...
second_optimizer = tf.train.AdamOptimizer(learning_rate=learning_rate).minimize(cost,var_list=[weights['wd2'],weights['out'],biases['bd2'],biases['out']])
# Evaluate model
...
saver.save(sess,'Saved\\CNN_200_pretrain')
...
saver.save(sess,'Saved\\CNN20_trained')
...
sample = np.zeros([1,200,3],dtype = float)
sample[0,:,:] = batch_x[0,:,:]
